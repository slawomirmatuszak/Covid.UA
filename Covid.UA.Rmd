...
Wśród państw sąsiadujących jedynie w Mołdawii jest znacząco większy przyrost nowych zakażeń w stosunku do liczby ludności, a na Węgrzech, Słowacji i w Rumunii zakażenia są na podobnym poziomie, jak na Ukrainie. 
...
plot_ly(d, x = ~data, y = ~test.per.th, color = ~Państwo,
        #width = 500, height = 500, 
        hoverinfo = 'text',
        text = ~paste('</br> data: ', data,
  ...
  config(locale = 'pl',modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "autoScale2d", "toImage", "lasso2d","select2d"),
         displaylogo=F)%>% 
  layout(yaxis = list(title="dzienne testy"), 
...
```{r fig.width=7, fig.height=4}
plot_ly(d, x = ~data, y = ~wykrywalnosc, color = ~Państwo,
        #width = 500, height = 500, 
        hoverinfo = 'text',
        text = ~paste('</br> data: ', data,
                      '</br> wykrywalność: ', paste0(round(wykrywalnosc*100,1), "%"),
...
#yaxis = list(tickformat = "%", title="wykrywalność")
```
