...
respiratory <- szpitale %>%
  select(data, respirator.sredni.covid, respirator.wysoki.covid)%>%
  group_by(data)%>%
  summarise(
    respirator = sum(respirator.sredni.covid, na.rm = T)+sum(respirator.wysoki.covid, na.rm = T)
  )%>%
  filter(!is.na(data))%>%
